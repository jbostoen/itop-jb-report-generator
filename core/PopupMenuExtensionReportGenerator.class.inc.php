...
class PopupMenuExtensionReportGenerator implements iPopupMenuExtension {
	...
	public static function ProcessTemplates($sTemplateType, DBObjectSearch $oFilter) {
		...
		if(isset($aReports[$sClassName]) == true && isset($aReports[$sClassName][$sTemplateType]) == true) {
			...
			foreach($aReports[$sClassName][$sTemplateType] as $aReportSettings) {
		
				$sTarget = (isset($aReportSettings['target']) == true ? $aReportSettings['target'] : '_blank');
				
				$sReportFile = $aReportSettings['file'];
				...
				self::$shortcut_actions .= (isset($aReportSettings['button']) == true && $aReportSettings['button'] == true ? ','.$sUID : '');
				...
				self::$menu_items[] = new URLPopupMenuItem($sUID, $aReportSettings['title'], $sURL, $sTarget);
